...
// kernel this emits two functions:
//
//   __vx_kentry_<name> — a naked entry stub placed in the .vx_entry section
//     (KEEP'd by the linker script). It is the address the VXSYMTAB footer
//     records and the KMU jumps to: it stashes the trampoline address in
//     s11 (callee-saved) and jumps to the shared __vx_cta_entry prologue in
//     libvortex2.a.
//   __vx_tramp_<name> — a vortex.kernel trampoline that runs the
//     once-per-launch args prologue (__vx_kernel_setup) and calls the
//     kernel.
//
// This replaces the legacy kernel_id switch table: each kernel is its own
// entry point, resolved by name via vx_module_get_kernel.
static std::string generateKernelEntries(
  ...
  ss << "/* Auto-generated by compile_vortex_program — per-kernel KMU entry\n"
        "   points for a multi-entry .vxbin. Do not edit. */\n"
  ...
  for (const auto& name : funcNames) {
    ss << "extern void " << name << "(void* args);\n"
       ...
       << "}\n"
       << "__attribute__((naked, section(\".vx_entry\"), used))\n"
       << "void __vx_kentry_" << name << "(void) {\n"
       << "  __asm__ volatile (\"lla s11, __vx_tramp_" << name << "\\n\"\n"
       << "                    \"j __vx_cta_entry\\n\");\n"
       << "}\n\n";
  }
  return ss.str();
